...
- `st.query_params.get("select")` を Python 側で検知 → **クリックした銘柄が属するセクション (保有/Watch) のタイル一覧の直下** に該当銘柄のチャートを展開 (Home最下部固定ではない)
...
- チャートは **Home最下部固定ではなく**、クリックした銘柄が属するセクション (保有銘柄 / Watch銘柄) の **タイル一覧の直下** に枠付きで展開される
- 開いているタイルは `.home-tile.selected` で青枠 + 薄い青背景 + 銘柄名太字
