...
### Homeの並び順 (常に騰落率 高い順・固定)
- Home は **常に「騰落率 高い順」固定** で表示 (並び替えselectboxは持たない・上部に「騰落率 高い順で表示」と小さく注記)
- 保有銘柄・Watch銘柄は **別グループのまま、それぞれの中で change_pct 降順**
- **取得失敗・騰落率なしの銘柄は常に最後**
- 表示順は cache せず、毎回 `watch_df` + 最新 `prices` から `_sort_home_by_change_pct_desc()` で計算 → 手動更新で価格が変われば自動的に最新の騰落率高い順に並ぶ
- 「今日強い銘柄を一目で見る」用途のため並び替え操作は不要とした (Portfolio タブの並び替えとは別物・Portfolioは従来どおり並び替え可)

### ヘッダーの余白
- タイトルは h4 (`#### 📈 Aさん株価ボード`)
- `.stApp .block-container` の **padding-top のみ** 調整 (PC 3.2rem / スマホ 1.0rem)・left/right/bottom や display は触らないため Portfolio/Settings のレイアウトは崩れない・PCは上端に詰まりすぎない自然な余白
...
### チャート形式 (1時間足=線グラフ / 日足・週足=ローソク足)
- **1時間足**: 見やすさ優先で **Close の線グラフ** (青 `#2563eb`・線幅2・マーカーなし)
  - **x軸は連番index** を使用 (実時間ベースだと夜間/土日/昼休み/非取引時間の空白が残り飛び飛びに見えるため)
  - これにより非取引時間の空白を詰めて連続した線として表示
  - ティックは **日付が変わる地点のみ** に間引き、短いラベル `M/D` を表示 (例: 5/22, 5/25, 5/26 …)
- **日足・週足**: **ローソク足** (`plotly.graph_objects.Candlestick`)・上昇=青 `#2563eb` / 下降=赤 `#dc2626`
  - OHLCのいずれかが欠ける場合のみ終値の折れ線にfallback し「OHLCデータなしのため折れ線表示」と注記
- 共通: Range slider 非表示・背景白・grid線薄め (`#eef1f5`)・価格軸は右側・高さ460px・margin (l/r/t=12, b=28)・出来高は非表示

### チャートの足種 (期間設定)

| 足種 | interval | period | 形式 | 本数(目安) |
|---|---|---|---|---|
| **1時間足** (初期表示) | `60m` | `5d` | 線グラフ (連番index) | 約31本 |
| **日足** | `1d` | `3mo` | ローソク足 | 約59本 |
| **週足** | `1wk` | `2y` | ローソク足 | 約105本 |

- 取得不可時 (1時間足など): 「1時間足データ取得不可。日足に切り替えてください」と表示・アプリは落とさない
- タイトルに「銘柄名 コード / 足種 / 取得期間」を表示 (例: `KDDI 9433 / 1時間足 / 5日`, `/ 日足 / 3ヶ月`, `/ 週足 / 2年`)

### Homeは手動更新後も常に最新の騰落率 高い順
- Home に並び替えUIはなく、**常に騰落率 高い順固定** (`home_sort_mode` 等の保持処理は廃止)
- 「🔄 手動更新」は価格キャッシュ (`fetch_one` / `fetch_history_cached`) のみクリアし、`open_chart_code` / Provider選択 / URLクエリ (`?select=`) は触らない
- Homeの表示順は **cacheせず**、毎回 `watch_df` + 最新 `prices` から `_sort_home_by_change_pct_desc()` で計算する
- → 手動更新で価格が変われば、**最新価格の騰落率 高い順で自動的に再表示**される (並び替え状態の保持は不要)。選択中チャートも維持される。
