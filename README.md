...
- 各セクション (保有銘柄 / Watch銘柄) を **小型タイル** で敷き詰め表示
- 1銘柄 = 1タイル (枠付き HTML カード) + **「📊 チャート」ボタン**
- レスポンシブ:
  - **PC**: 「PC列数」ラジオ (2/3/4/5/6) で切替 (デフォルト 4列)
  - **スマホ**: CSSメディアクエリで **2列固定** (狭幅でも縦1列に潰れない)
- タイルの表示内容:
  - 銘柄名 / コード / 現在価格 / **🔵 (+x.xx%) / 🔴 (-x.xx%) / ⚪ (0.00%)** (色付き)
  - 価格が `manual_prices.csv` 由来の場合は「※ 手動値」と小さく表示
- 銘柄を選択するには:
  - タイル下の「📊 チャート」ボタンを押す
  - `selected_home_code` が session_state に保存され、**Home 下部の「選択中チャート」セクションに1つだけチャートが表示**
  - 別タイルを押せばチャートが切り替わる
- 選択中チャート:
  - **1時間足 / 日足 / 週足 切替** (初期=1時間足)
  - Plotly 折れ線・Yahoo / TradingView 外部リンク
- 取得失敗銘柄はタイル内に「取得失敗」と表示・他銘柄は継続表示
- 画面上部の大きな警告は出さず、`※ 一部銘柄 (n件) は取得に失敗` の小キャプションのみ
