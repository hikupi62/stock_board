...
### Home画面の表示仕様 (世界の株価風タイル)
- 各セクション (保有銘柄 / Watch銘柄) を **小型カードタイル** で敷き詰め表示
- **1銘柄 = 1ボタン (カード風)** ・別途「📊 チャート」ボタンは廃止
- **タイル自体をタップ/クリックで選択** → Home下部に1つだけチャート表示
- **列数は画面幅で自動調整** (CSS `grid-template-columns: repeat(auto-fit, minmax(...))`)
  - **スマホ (≤640px)**: 最小幅 105px (2-3列で折り返し)
  - **PC (≥641px)**: 最小幅 140px (4-6列で折り返し)
  - 列数選択ラジオは廃止
- **タイル表示内容** (コードは省略・優先度高い情報のみ):
  - 銘柄名
  - 現在価格
  - **🔵 +x.xx% / 🔴 -x.xx% / ⚪ 0.00%** (絵文字マーカー)
  - 手動値の場合は「※ 手動値」と1行追加
  - 取得失敗時は「取得失敗」と表示
- **選択中タイル**: Streamlit の primary ボタン (青枠 + 薄青背景) で強調
- **選択中チャート (Home下部)**:
  - 「📊 選択中チャート」セクションに1つだけ
  - **1時間足 / 日足 / 週足 切替** (初期=1時間足)
  - Plotly 折れ線・Yahoo / TradingView 外部リンク
- 取得失敗銘柄: 画面上部に大きな警告は出さず、小さなキャプションのみ (タイル内に「取得失敗」)
