...
### Homeの並び替えは手動更新後も維持＋最新価格で再ソート
- Home並び替え selectbox は固定 `key="home_sort_mode"` で `st.session_state` に保持
  - **`index=` は使わない** (`index` と `key` の併用は default と session_state の二重指定になり、`st.rerun()` 直後に selectbox の戻り値が表示値とズレる原因になるため)。`session_state` を初回のみ「登録順」で初期化し、selectbox は `key` のみで作る。
- 「🔄 手動更新」は価格キャッシュ (`fetch_one` / `fetch_history_cached`) のみクリアし、**session_state (`home_sort_mode` / `open_chart_code` / Provider選択) や URLクエリ (`?select=`) は触らない**
- Homeの表示順は **cacheせず**、毎回 `watch_df` + 最新 `prices` + 現在の `home_sort_mode` から `_sort_home_sub()` で計算する
- → 手動更新で価格が変われば、selectbox の値 (例: 騰落率 高い順) を保ったまま **最新価格で必ず再ソート**される。選択中チャートも維持される。
