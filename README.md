...
### Homeのレイアウト (PC=左タイル/右チャート・スマホ=タイル下チャート)
- **PC**: `st.columns([0.58, 0.42])` で **左55-60%にタイル / 右40-45%にチャート** の2カラム
  - 銘柄クリック後にページ上部へ戻っても、右カラム上部にチャートが見える (スクロールせず価格一覧とチャートを同時確認)
  - 未選択時は右カラムに「📊 銘柄をクリックするとチャートを表示します」
- **スマホ**: Streamlitのレスポンシブで2カラムが縦積みになり、**タイル一覧の下にチャート**が表示される (従来どおり・見切れない)
...
### チャート形式 (ローソク足・OHLC欠損時は折れ線fallback)
- **原則ローソク足** (`plotly.graph_objects.Candlestick`)・yfinance履歴の Open/High/Low/Close を使用
- 上昇 (close>open) = 青 `#2563eb` / 下降 = 赤 `#dc2626` (アプリ配色に統一)
- Range slider は非表示 (`xaxis_rangeslider_visible=False`)・余白少なめ・価格軸は右側
- **OHLCのいずれかが欠ける場合は終値 (Close) の折れ線にfallback** し、「OHLCデータなしのため折れ線表示」と小さく注記
- 出来高は初期版では非表示

### チャートの足種 (足種ごとの表示期間)
- **1時間足**: yfinance `interval=60m, period=30d` (**初期表示**) — 日本株では取得不可・欠損のケースあり (取得不可時は「1時間足データ取得不可。日足に切り替えてください」と表示)
- **日足**: yfinance `interval=1d, period=6mo` (半年)
- **週足**: yfinance `interval=1wk, period=3y` (3年)
- タイトルに「銘柄名 コード / 足種 / 取得期間」を表示 (例: `KDDI 9433 / 1時間足 / 30日`, `/ 日足 / 半年`, `/ 週足 / 3年`)
