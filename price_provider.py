...
class PriceData:
    ...
    source: str = "yfinance"  # "yfinance" / "fast_info" / "info" / "manual" など
        ...
        try:
            ...
            for _, r in df.iterrows():
                ...
                try:
                    ...
                    price = None
                try:
                    prev = float(r.get("previous_close")) if r.get("previous_close") not in (None, "", "nan") else None
                except Exception:
                    prev = None
...
class YFinanceProvider(PriceProvider):
    ...
    def fetch_history(
        ...
        """チャート用ヒストリ取得。interval を指定可能。フォールバック付き。

        interval 例: "60m" (1時間足), "1d" (日足), "1wk" (週足)
        period 例: "60d" / "3mo" / "6mo" / "1y" / "2y" / "5y"

        取得優先順:
          1. yfinance.history(period, interval)
          2. yfinance.history(period="5d", interval="1d") の最新Close (fallback)
          3. ticker.fast_info の last_price / previous_close
          4. ticker.info の regularMarketPrice / previousClose
          5. data/manual_prices.csv の手動値
        ...
        try:
            ...
            # ----- 2) 5日日足fallback (interval違い・低流動性銘柄向け) -----
            try:
            ...
            try:
                ...
                # fast_info はdict-like or attribute-likeどちらにも対応
                def _g(o, k):
                    try:
                        v = o.get(k)
                    except Exception:
            ...
            # yfinance自体のimport/Tickerで失敗
            pass

        # ----- 5) manual_prices.csv -----
        try:
            manual = _load_manual_prices()
            # symbol (例: "1687.T") と code (例: "1687") の両方で探す
